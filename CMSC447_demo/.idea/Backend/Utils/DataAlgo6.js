...
function checkGraduationRequirements(allCourses, track) {
    ...
    switch (track) {
        ...
        case "Computer Engineering - Electronic Systems":
            required = [
                /* …your CE core + spec‐track courses… */
            ];
            break;
        ...
        case "Chemical Engineering - General":
            required = [
                // Year 1
                "CHEM 101",    // Prin. of Chemistry I
                "MATH 151",    // Calc & Analytic Geom I
                "ENES 101",    // Intro to Engineering
                "ENME 110",    // Statics

                // Year 1 Spring
                "CHEM 102",    // Prin. of Chemistry II
        ...
        case "Information Systems - General":
            required = [
                // Core IS courses
                "IS 300","IS 310","IS 410","IS 420",
                "IS 425","IS 436","IS 450","IS 451",
                // Math & Stats
                "MATH 155","MATH 215","STAT 351",
                // Programming foundations (choose one of each pair)
                "IS 147","CMSC 201","IS 247","CMSC 202",
                // One additional approved programming elective
                "IS 246","IS 413","IS 448","IS 471","IS 472",
                // Azdmin. Science
                "MGMT 210","ECON 101","ECON 102",
                "ECON 121","ECON 122","ENGL 393","IS 369"
            ];
            break;
        ...
        case "Business Technology Administration - B.A.":
            required = [
                // Year One
                "IS 125",    // Systems Logic & Structured Design
                "IS 296",    // (alternative to IS 125)
                ...
                // Year Three
                "IS 303",    // Fund. of Human–Computer Interaction
                "IS 325",    // Introduction to Management Science
                "STAT 351",  // Applied Statistics for Business & Econ.

                // Year Four
                "IS 350",    // Business Communications Systems
                "ENGL 393",  // Technical Writing (WI)
                "IS 440",    // Integrating Tech. in Bus. Processes
                "IS 438",    // Project Management
                "MGMT 489"   // Seminar in Mgmt. & Admin.
            ];
            break;

        // ── fallback ──
        default:
            // default to CS General if track is unrecognized
            required = [
                "CMSC 201","CMSC 202","CMSC 203",
                "CMSC 313","CMSC 304","CMSC 331",
                "CMSC 341","CMSC 411","CMSC 421",
    ...
    required.forEach(code => {
        if (!has(code)) {
            result.missing.push(`Missing: ${code}`);
        }
    });
    ...
    // other 400-level tech electives (excl. 404,495–499 & CS-list)
    ...
    if (!hasScienceSequence()) {
        result.missing.push("Science sequence (BIOL/CHEM/PHYS)");
    }
    if (!validateLang201(normalized)) {
        result.missing.push("Foreign language 201-level course required");
    }
    if (upperLevelCredits < 45) {
        result.missing.push(`Only ${upperLevelCredits}/45 upper-level credits`);
    }
    if (!validateArtsHumanities(normalized)) {
        result.missing.push("Arts & Humanities: 3 courses from 2+ disciplines");
    }
    if (!validateCulture(normalized)) {
        result.missing.push("Culture requirement: 1 approved course");
    }
...
const langPrefixes = ["FREN", "GERM", "SPAN", "JPNS", "KORE", "CHIN", "LATN", "HEBR", "ARBC", "RUSS", "GRK", "ITAL"];
...
function validateCulture(coursesTaken) {
    return coursesTaken.some(course => cultureCourses.includes(course));
}
